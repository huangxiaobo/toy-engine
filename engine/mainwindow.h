...
private:
    ...
    bool m_dock_space_initialized = false;
    ...
    bool m_show_shadow_depth_map = false;
    // 阴影属性面板开关（阴影开关、光源摄像机参数、深度贴图预览等全局阴影设置）
    // 默认显示：阴影参数属于常用调试信息，启动即展示，无需先通过菜单打开
    bool m_show_shadow_properties = true;
    // 调试属性面板开关（DebugDraw 光源线框开关等渲染调试项）
    bool m_show_debug_properties = true;

    // 阴影深度贴图可视化面板的显示状态：
    // 深度贴图是 GL_DEPTH_COMPONENT 只写格式，ImGui 颜色采样器难以可靠显示，
    // 故每次面板打开时把深度读回 CPU、归一化为灰度并上传到一张 RGBA8 纹理再展示。
    unsigned int m_shadow_depth_preview_tex = 0;   // 承载归一化灰度预览的 RGBA8 纹理（缓存复用）
    int m_shadow_depth_preview_w = 0;              // 预览纹理宽度（读回分辨率）
    int m_shadow_depth_preview_h = 0;              // 预览纹理高度（读回分辨率）
    unsigned int m_last_displayed_depth_tex = 0;   // 上次显示的源深度贴图 ID，用于缓存失效判断

    // 当前选中的资源（union 风格：指针 + 类型字符串）
    // 支持类型："Model", "Light", "Camera", "Terrain", "SkyDome", "Particle"
    void* m_selected_object = nullptr;
    std::string m_selected_object_type;
    int m_selected_particle_index = -1;   // 粒子系统选中索引（粒子用索引而非指针）

    // 鼠标状态
    bool m_mouse_left_pressed = false;
    bool m_mouse_right_pressed = false;
    double m_current_mouse_x = 0.0;
    double m_current_mouse_y = 0.0;
    // 左键按下时的光标位置（松开时用于区分"点击"与"拖拽"，位移小于阈值视为点击并触发拾取）
    double m_mouse_down_x = 0.0;
    double m_mouse_down_y = 0.0;
