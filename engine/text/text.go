...
func NewText(content string, pix float32) (*Text, error) {
	f := &Text{
		vbo:    &VboData{},
		loaded: false,
		text:   content,
	}
	contents, err := ioutil.ReadFile(FontFile)
	if err != nil {
		fmt.Println("Could not read font file: " + FontFile)
		panic(err)
	}
	fontFace, err := truetype.Parse(contents)
	if err != nil {
		fmt.Println("Could not parse font file: " + FontFile)
		panic(err)
	}
