 ...
 *   SpecularScale            —— specular 乘数：方向光传 1.0；点光/聚光传各自的
 *                               1.0/atten（及锥角平滑系数），保证高光不逃逸距离衰减
 *
...
vec4 CalcPointLight(int idx, vec3 N) {
    ...
    // 物理衰减：1 / (Kc + Kl*d + Ke*d^2)，先算出来供漫反射和镜面高光共用
...
vec4 CalcSpotLight(int idx, vec3 N) {
    ...
    // 与点光源一致先算衰减；再算内锥->外锥线性平滑过渡系数，两者同乘给各分量
